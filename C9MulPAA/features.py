...
def extract_stats(temp_list):
    return [max(temp_list), min(temp_list), np.std(temp_list),stats.scoreatpercentile(temp_list, 25),stats.scoreatpercentile(temp_list, 75),
            stats.kurtosis(temp_list),np.median(temp_list), len(find_peaks(temp_list)[0]),stats.skew(temp_list)]
                ...
                if eval(self.paa):
                    ...
                    for seg in self.segs_:
                        ...
                        paa_per_seg = PiecewiseAggregateApproximation(n_segments=s)\
                                .fit_transform([dim.values]).flatten()
